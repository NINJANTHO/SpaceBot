module.exports.run = async (client, message, args, data) => {
    ...
    if(args[0] === "add") {
        ...
        message.channel.send(`<:modo:821417956130553876> **${channel} est désormais ignoré par l'anti spam.**`);
    } else if(args[0] === "remove") {
        if(!channel) return message.channel.send(`<:information:820697291911987211> **Attention, merci de spécifier un salon à ignorer, essayez en donnant une ID ou en mentionnant le salon**`);
        ...
        data.markModified("plugins.protection.antispam");
        data.save();
        message.channel.send(`<:modo:821417956130553876 **${channel} n'est désormais plus ignoré par l'antispam**`);
    } else return message.channel.send(`<:information:820697291911987211> **Attention, vous n'utilisez pas la commande correctement.\nFaites \`${data.prefix}ignore-channel add <salon>\` pour rajouter un salon, et \`${data.prefix}ignore-channel remove <salon>\` pour en retirer un.`);
}
module.exports.help = {
    name: "ignore-channel",
