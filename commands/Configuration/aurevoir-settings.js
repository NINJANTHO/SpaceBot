...
module.exports.run = async (client, message, args, data) => {
    if(!data.plugins.goodbye.enabled) return message.channel.send(`<:general:820696803363782696> **Le plugin n'est pas activer, veuillez l'activer**`);
    ...
    if(args[0] === "channel") {
        if(args[1] && (message.mentions.channels.first() || message.guild.channels.cache.get(args[1]))) {
            ...
            return message.channel.send('<:information:820697291911987211> **Je viens de modifier le salon d\'aurevoir.**');
        } else {
            ...
            c.on("collect", async msg => {
                ...
                if(channel.id == data.plugins.goodbye.channel) return message.channel.send('**Ce salon est déjà défini comme salon d\'aurevoir*');
