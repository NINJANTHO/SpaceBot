...
module.exports.run = (client, message, args, data) => {
    ...
    if(user.id == message.author.id) return message.channel.send(`<:information:820697291911987211> **Attention, vous ne pouvez pas vous expulser vous même**`);
    ...
    if(message.guild.ownerID !== message.author.id) {
        if(moderatorPosition <= memberPosition) return message.channel.send(`<:information:820697291911987211> **Attention, vous ne pouvez pas expulser ce membre.**`);
    }

    if(!member.kickable) return message.channel.send(`<:information:820697291911987211> **Attention, je n'ai pas les permissions suffisantes pour expulser ce membre, vérifiez que mon rôle est au dessus du membre à kick, et réessayez.**`);
