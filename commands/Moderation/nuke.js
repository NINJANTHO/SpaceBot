            ...
            setTimeout(async () => {
                await channel.delete(`Nuke by ${message.author}`).then(async (ch) => {
                    ...
                    await message.guild.channels.create(ch.name, {
                        ...
                        console.error(err);
                        message.author.send(`<:information:820697291911987211> **Attention, une erreur est survenue lors de la création du salon.**`).catch(() => {});
                    });
                }).catch(err => {
                    if(err.code === 50074) return message.channel.send(`<:information:820697291911987211> **Attention, impossible de supprimer un salon requis pour la communauté**`);
                    console.error(err);
                    message.channel.send(`Une erreur est survenue : \n\`\`\`js${err}\n\`\`\``);
                });
            }, 5000)
        } else return message.channel.send(`<:information:820697291911987211> **Attention, je n'ai pas les permissions de supprimer ce salon.** `);
    } else {
